...
export async function buscarVendasDoCliente(
  ...
  if (!canal?.ativo) {
    ...
    // TEXTO. Ela fazia isso, com as duas grafias (com e sem acento), porque
    // ninguém sabia qual chegaria — o sintoma de decidir comportamento pela
    // redação de uma frase. Melhore o texto e a tela quebra em silêncio.
    return { pedidos: [], aviso: "Cliente não conectado ao Mercado Livre.", precisaConectar: true };
  }
  ...
  if (!resposta.ok) {
    ...
    // loja inteira zerada, com sete cartões de R$ 0, em cima de uma lista que
    // ninguém conseguiu ler. É a ausência virando afirmação, de novo.
    return {
      pedidos: [],
