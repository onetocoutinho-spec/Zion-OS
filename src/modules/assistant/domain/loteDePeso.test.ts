...
const rodar = (args: Record<string, unknown>, c = ctx) =>
  executarFerramenta({ nome: "propor_gravacao", args }, c);

test("o fluxo INDIVIDUAL continua igual — um produtoId, uma proposta", () => {
  const r = rodar({ produtoId: "h1", campo: "peso", valor: "420", unidade: "g" });
  assert.equal(r.proposta?.tipo, "pronta");
  assert.equal(r.escopo, undefined);
});

test("lote de peso monta o escopo com quem está SEM peso", () => {
  const r = rodar({ produtoIds: ["h1", "h2", "h3"], campo: "peso", valor: "420", unidade: "g" });
  assert.ok(r.escopo);
...
test("a frase que o lojista lê traz os 47 e os que ficam de fora", () => {
  const r = rodar({ produtoIds: ["h1", "h2", "h3"], campo: "peso", valor: "420", unidade: "g" });
...
test("o modelo recebe CONTAGEM e AMOSTRA, nunca a lista inteira", () => {
...
test("CUSTO EM LOTE É RECUSADO — e a recusa diz por quê", () => {
  ...
  const r = rodar({ produtoIds: ["h1", "h2"], campo: "custo", valor: "38,70", unidade: "reais" });
...
test("custo INDIVIDUAL continua funcionando", () => {
  const r = rodar({ produtoId: "h1", campo: "custo", valor: "38,70", unidade: "reais" });
  assert.equal(r.proposta?.tipo, "pronta");
});

test("id que o modelo alucinou não vira alvo", () => {
  const r = rodar({ produtoIds: ["h1", "inexistente"], campo: "peso", valor: "420", unidade: "g" });
  assert.deepEqual(r.escopo!.incluidos.map((c) => c.id), ["h1"]);
});

test("todos os alvos inexistentes: não propõe nada", () => {
  const r = rodar({ produtoIds: ["x", "y"], campo: "peso", valor: "420", unidade: "g" });
  assert.equal((r.saida as { montada: boolean }).montada, false);
  assert.equal(r.escopo, undefined);
});

test("ninguém sem peso NO LOTE: não propõe, e explica", () => {
  const r = rodar({ produtoIds: ["h3", "h3"], campo: "peso", valor: "420", unidade: "g" });
  assert.equal((r.saida as { montada: boolean }).montada, false);
  assert.match((r.saida as { motivo: string }).motivo, /Não há o que aplicar/);
});

test("UM id em produtoIds vai pelo caminho individual — e acha o alvo", () => {
  ...
  const r = rodar({ produtoIds: ["h1"], campo: "peso", valor: "420", unidade: "g" });
  assert.equal(r.proposta?.tipo, "pronta");
  assert.equal(r.proposta.alvo.id, "h1");
});

test("individualmente, SOBRESCREVER quem já tem peso é permitido — e é o ponto", () => {
  ...
  const r = rodar({ produtoId: "h3", campo: "peso", valor: "420", unidade: "g" });
  assert.equal(r.proposta?.tipo, "pronta");
});

test("a vírgula decimal sobrevive no lote", () => {
  const r = rodar({ produtoIds: ["h1", "h2"], campo: "peso", valor: "0,42", unidade: "kg" });
...
test("a 48ª variante que apareceu DEPOIS não entra — e não invalida", () => {
...
test("TUDO OU NADA: um alvo preenchido no meio-tempo invalida a proposta inteira", () => {
...
test("alvo que SUMIU do tenant vira null e invalida", () => {
...
test("Proposal de lote de OUTRO TENANT não executa", () => {
...
test("confirmação duplicada de LOTE não executa de novo", () => {
