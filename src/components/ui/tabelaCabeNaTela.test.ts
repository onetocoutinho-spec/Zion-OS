...
test("o CABEÇALHO continua sem quebrar — ali o texto é curto", () => {
  ...
  // Desde o item D existe um `<th>` ANTES dele — o da caixa de marcação, que
  // não tem texto e portanto não tem o que quebrar. Ancorado em "o primeiro
  // `<th>`", este teste passou a medir a célula errada e reprovou uma mudança
  // correta. Sentinela ancorada em posição envelhece com o layout.
  const codigo = semComentarios(TABELA);
