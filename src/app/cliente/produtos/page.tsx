 ...
 *
 * As duas estavam escritas à mão, em lugares distantes: as opções aqui em cima
 * e a classificação lá dentro do filtro. Ao trocar "Score" por "Nota" (item C)
 * renomeei uma e não a outra, e o filtro passou a comparar "Alta (70+)" com
 * "Alto (70+)" — nenhum produto casava, e a tela ficava vazia sem dizer por quê.
 *
 * O tipo de retorno é a garantia: se as duas listas divergirem de novo, quem
 * reclama é o compilador, não a lojista.
 */
...
export default function ClienteProdutos() {
  ...
  const filtrados = useMemo(() => {
    ...
    return (produtos ?? []).filter((p) => {
      ...
      if (fScore !== "Todos") {
        if (faixaDaNota(scorePorProduto.get(p.id) ?? null) !== fScore) return false;
      }
