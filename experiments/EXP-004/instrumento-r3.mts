// EXP-004 — RODADA 3. Instrumento experimental. NÃO É PRODUTO.
//
...
const ESQUEMA = {
  ...
  required: ["entendeu", "perguntar", "marca", "estadoDePeso", "termosDoProduto", "relacao", "interpretacao"],
...
// ── Resolvedor ──────────────────────────────────────────────────────────────
function resolver(marca: string, estado: string, termosDoProduto: string[], relacao: string) {
  const classificadas = termosDoProduto.map((t) => ({ termo: t, ...classificar(t) }));
...
for (const caso of CASOS) {
  ...
  const r = invalido
    ...
    : resolver(bruto.marca, bruto.estadoDePeso, bruto.termosDoProduto ?? [], bruto.relacao);
  ...
  if (COM_RESTRICAO_REAL.includes(caso.id)) { n2++; if ((bruto.termosDoProduto ?? []).some((t: string) => classificar(t).classe !== "generica")) m2++; }
  // M3
  if (conjuntoOk) m3++;
  ...
  // ── Consistência entre eixos (congelada antes da execução) ───────────────
  // DETECÇÃO, não prevenção: o classificador NÃO conhece estas palavras.
  // Só medimos se um termo duplica um eixo já extraído e se isso alterou o
  // desfecho ou criou fronteira falsa.
  const dupl = (bruto.termosDoProduto ?? []).filter((t: string) => {
    const n = semAcento(t);
    return (bruto.marca && n === semAcento(bruto.marca)) || n.includes("peso");
  });
  const fronteiraFalsa = r.fronteira.some((f) => dupl.includes(f.termo));
  const desfechoAlterado = fronteiraFalsa && esp.desfecho !== 4;
  if (dupl.length) inconsistencias.push(
    `${caso.id}: termo(s) [${dupl.join(", ")}] duplica(m) eixo já extraído` +
    ` · fronteiraFalsa=${fronteiraFalsa} · desfechoAlterado=${desfechoAlterado}`);
  if (fronteiraFalsa || desfechoAlterado) falhaConsistencia = true;
  ...
  console.log([
    ...
    `criterio cru:     marca="${bruto.marca}" estado=${bruto.estadoDePeso} termosDoProduto=[${(bruto.termosDoProduto ?? []).join(", ")}] relacao=${bruto.relacao}`,
...
console.log("");
console.log("# CONSISTENCIA ENTRE EIXOS");
console.log(inconsistencias.length ? inconsistencias.join(String.fromCharCode(10)) : "nenhuma duplicacao entre eixos detectada");
console.log(`veredito consistencia: ${falhaConsistencia ? "FALHA" : "OK"}`);
