...
| S-30 | **Delegation Audit** *(germinada no perímetro da DI — E5.9, 2026-07-23)* — humanos assinam (`aprovadoPor/Em`); as Ofertas assinam **completo e versionado** (`AssinaturaDeSistema`: autor canônico + 4 versões — migrações 025/026); componentes futuros obrigados via `assinaturaDe()`. **Resíduo fora da AIL:** as execuções autônomas do domínio (guia, token, fallback de categoria) seguem sem rastro — candidato a ciclo próprio | `system-authorship.ts` · `ofertas` | assinar as execuções do domínio — pré-requisito para elas subirem de nível |
