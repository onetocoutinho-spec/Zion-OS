...
| **E5.9** | **System Authorship** | implementada e validada | De "quem agiu?" para **"qual versão de quê agiu?"**: assinatura versionada em toda Oferta (versões = citações de artefatos congelados); componentes futuros obrigados por regra; legado = NULL honesto. Fecha a S-30 no perímetro da DI. |
| **E5.10a** | **Knowledge Maturation** | implementada e validada | A ADR-002 executada ao pé da letra: fatos append-only + fotografia congelada; **sem assinatura humana não existe fato**; a régua de campo recomeça a cada versão; o sistema propõe e sinaliza — jamais promove ou rebaixa. |
| **E5.10b** | **Delegation Runtime** | implementada e validada | Autonomia com grant: Knowledge VIGENTE (**jamais Confidence**) + autoridade humana + executor assinado. A execução registra oferta assinada e **herda toda a auditoria existente**; rebaixar revoga o futuro; o passado permanece. **ROADMAP E7 COMPLETO — o ciclo aprender→institucionalizar→autorizar→agir fechou, com o humano assinando cada mudança de natureza.** |
