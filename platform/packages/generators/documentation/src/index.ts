...
// Compiler 011 — Documentation: apresentação legível. Usa a face `.` do Símbolo. A Incompletude é
// exibida como ESTADO (5.5.1), nunca preenchida.
...
export function generateDocumentation(validated: ValidatedIR): GeneratorOutput {
  const diagnostics: Diagnostic[] = [];
  const lines: string[] = ['# Tokens', ''];
  for (const node of ordered(validated.ir.nodes)) {
    lines.push('## ' + node.symbol);
    if (node.matter !== undefined) lines.push('- Materia: ' + node.matter);
    else lines.push('- Materia: unresolved');
    if (node.destination !== undefined) lines.push('- Destinacao: ' + node.destination.consumer);
    for (const resolution of node.resolutions) {
      const context = resolution.context === undefined ? '(invariante)' : resolution.context;
      if (resolution.completeness === 'incomplete') {
        diagnostics.push(incompleteDiagnostic(node));
        lines.push('- ' + context + ': Incompleto');
        continue;
      }
      lines.push('- ' + context + ': ' + JSON.stringify(resolution.value));
    }
    lines.push('');
  }
